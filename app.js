...
app.post('/users', async function (req, res) {
	...
	var roleOrg = req.body.roleOrg;
	logger.debug('End point : /users');
	logger.debug('User name : ' + username);
	logger.debug('Org name  : ' + orgName);
	logger.debug('Role Here  : ' + roleOrg);
	if (!username) {
	...
	if (!roleOrg) {
		res.json(getErrorMessage('\'roleOrg\''));
		return;
	}
	var token = jwt.sign({
	...
	let response = await helper.getRegisteredUser(username, orgName, roleOrg, true);
